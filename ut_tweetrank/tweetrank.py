...
class TweetRank:
    ...
    def __init__(self, args):
        ...
        if args is not None:
            ...
            print("No args supplied to TweetRank.")

        # instance variables
        self.output_filename = "ut_ranked.txt"
        self.output_filename_clean = "ut_ranked_clean.txt"
        self.sorted_cropped_rankings = defaultdict(list)
        self.rankings_output = list()

    def PerformAction(self):
        if self.ACTION == 'rank':
            self.TweetRankFromFile(self.args.file)

    def TweetRankFromFile(self, fname):
        ...
        self.TweetRank(unidentified_terms)
        ...
        with open(self.output_filename_clean, 'w') as outputfile_clean:
            for ut in unidentified_terms:
                ...
                for i in range(0, number_rankings):
                    outputfile_clean.write(str(self.sorted_cropped_rankings[ut][i][0]) + '\t')
    ...
    def SortCropRankings(self, ranked_tweets, unidentified_terms):
...
def main(arguments):
    parser = argparse.ArgumentParser(prog="TweetRank", description=__doc__,
    ...
    tweetranker = TweetRank(args)
    tweetranker.PerformAction()
